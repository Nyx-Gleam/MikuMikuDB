...
class SongVariantDialog:
    def __init__(self, parent, variant_data=None):
        ...
        self.dialog.title("オーディオバリアントの設定")
    ...
    def create_widgets(self):
        ...
        # Title
        ttk.Label(main_frame, text="オーディオバリアントの設定", font=('Arial', 12, 'bold')).grid(row=0, column=0, columnspan=2, pady=(0, 10))
        
        # Fields
        fields = [
            ("バリアント名:", self.name_var),
            ("英語名:", self.name_en_var),
            ("代替英語名:", self.name_en2_var),
            ("ローマ字名:", self.name_ro_var),
            ("バリアントアーティスト:", self.vocal_disp_var),
            ("英語アーティスト名:", self.vocal_disp_en_var)
        ]
        ...
        ttk.Button(button_frame, text="確定", command=self.accept).pack(side=tk.LEFT, padx=(0, 10))
...
class SongConfigDialog:
    ...
    def create_widgets(self):
        ...
        ttk.Button(button_frame, text="確定", command=self.accept).pack(side=tk.RIGHT, padx=(10, 0))
    ...
    def create_basic_tab(self):
        ...
        fields = [
            ...
            ("オリジナル名:", self.song_name_var),
            ("英語名:", self.song_name_en_var),
            ("代替英語名:", self.song_name_en2_var),
            ("ひらがな名:", self.song_name_reading_var),
            ("ローマ字名:", self.song_name_ro_var),
            ("BPM:", self.bpm_var),
            ("日付 (YYYYMMDD):", self.date_var),
            ("サビ開始時間:", self.sabi_start_var),
            ("サビ再生時間:", self.sabi_play_var)
        ]
    ...
    def create_difficulty_tab(self):
        ...
        self.notebook.add(diff_frame, text="難易度")
        ...
        difficulties = [
            ("イージー", self.easy_var, self.easy_level_var),
            ("ノーマル", self.normal_var, self.normal_level_var),
            ("ハード", self.hard_var, self.hard_level_var),
            ("エクストリーム", self.extreme_var, self.extreme_level_var),
            ("エクストラエクストリーム", self.extreme_extra_var, self.extreme_extra_level_var)
        ]
    ...
    def create_performers_tab(self):
        ...
        ttk.Button(perf_buttons, text="削除", command=self.remove_performer).pack(side=tk.LEFT)
        
        # Character selection dropdown
        ttk.Label(perf_frame, text="キャラクター選択:").pack(anchor=tk.W, pady=(20, 5))
    ...
    def create_songinfo_tab(self):
        ...
        self.notebook.add(info_frame, text="楽曲情報")
        ...
        labels_en = {
            'arranger_en': 'アレンジャー (英語):',
            'guitar_player_en': 'ギタリスト (英語):',
            'lyrics_en': '作詞者 (英語):',
            'manipulator_en': 'マニピュレーター (英語):',
            'music_en': '作曲者/アーティスト (英語):',
            'pv_editor_en': 'PV編集者 (英語):'
        }
    ...
    def create_variants_tab(self):
        ...
        self.notebook.add(variants_frame, text="オーディオバリアント")
        
        ttk.Label(variants_frame, text="設定されたオーディオバリアント:").pack(anchor=tk.W)
        
        # Variants list
        self.variants_tree = ttk.Treeview(variants_frame, columns=('name', 'name_en', 'artist'), show='headings', height=8)
        self.variants_tree.heading('name', text='オリジナル名')
        ...
        ttk.Button(variants_buttons, text="バリアント追加", command=self.add_variant).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(variants_buttons, text="バリアント編集", command=self.edit_variant).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(variants_buttons, text="バリアント削除", command=self.remove_variant).pack(side=tk.LEFT)
    ...
    def edit_variant(self):
        ...
        if not selection:
            messagebox.showwarning("警告", "編集するバリアントを選択してください")
            return
    ...
    def remove_variant(self):
        selection = self.variants_tree.selection()
        if not selection:
            messagebox.showwarning("警告", "削除するバリアントを選択してください")
            return
    ...
    def accept(self):
        ...
        if not self.pv_id_var.get().strip():
            messagebox.showerror("エラー", "PV IDは必須です")
            return
...
class ProjectDivaModEditor:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("MikuMikuDB エディター")
    ...
    def create_widgets(self):
        # Main title
        title_label = ttk.Label(self.root, text="MikuMikuDB エディター", font=('Arial', 16, 'bold'))
        title_label.pack(pady=10)
        
        # Pack information frame
        pack_frame = ttk.LabelFrame(self.root, text="楽曲パック情報", padding="10")
        pack_frame.pack(fill='x', padx=10, pady=5)
        
        # Pack name
        ttk.Label(pack_frame, text="楽曲パック名:").grid(row=0, column=0, sticky=tk.W, pady=2)
        ...
        # Songs frame
        songs_frame = ttk.LabelFrame(self.root, text="楽曲", padding="10")
        ...
        self.songs_tree.heading('name', text='オリジナル名')
        ...
        ttk.Button(songs_buttons, text="楽曲追加", command=self.add_song).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(songs_buttons, text="楽曲編集", command=self.edit_song).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(songs_buttons, text="楽曲削除", command=self.delete_song).pack(side=tk.LEFT, padx=(0, 5))
        
        # Main buttons
        main_buttons = ttk.Frame(self.root)
        main_buttons.pack(fill='x', padx=10, pady=10)
        
        ttk.Button(main_buttons, text="ファイル生成", command=self.generate_file).pack(side=tk.RIGHT, padx=(10, 0))
        ttk.Button(main_buttons, text="設定読込", command=self.load_config).pack(side=tk.RIGHT, padx=(10, 0))
        ttk.Button(main_buttons, text="自動保存読込", command=self.load_autosave).pack(side=tk.RIGHT, padx=(10, 0))
        ttk.Button(main_buttons, text="設定保存", command=self.save_config).pack(side=tk.RIGHT, padx=(10, 0))
    ...
    def edit_song(self):
        ...
        if not selection:
            messagebox.showwarning("警告", "編集する楽曲を選択してください")
            return
    ...
    def delete_song(self):
        selection = self.songs_tree.selection()
        if not selection:
            messagebox.showwarning("警告", "削除する楽曲を選択してください")
            return
        
        if messagebox.askyesno("確認", "この楽曲を削除してもよろしいですか？"):
            item = selection[0]
            index = self.songs_tree.index(item)
    ...
    def generate_file(self):
        if not self.pack_name_var.get().strip():
            messagebox.showerror("エラー", "楽曲パック名を入力してください")
            return

        if not self.songs:
            messagebox.showerror("エラー", "楽曲を少なくとも1つ追加してください")
            return
        ...
        if filename:
            try:
                with open(filename, 'w', encoding='utf-8') as f:
                    f.write(content)
                messagebox.showinfo("成功", f"ファイルを正常に生成しました: {filename}")
            except Exception as e:
    ...
    def generate_file_content(self):
        ...
        content.append(f"# {self.pack_name_var.get()} 追加譜面")
        content.append("#")
    ...
    def save_config(self):
        if not self.songs:
            messagebox.showwarning("警告", "保存する楽曲がありません")
            return

        filename = filedialog.asksaveasfilename(
            defaultextension=".pdpack",
            filetypes=[
                ("Project Diva パックファイル", "*.pdpack")
            ],
    ...
    def load_config(self):
        ...
        if filename:
            try:
                ...
                self.update_songs_display()
                messagebox.showinfo("成功", f"設定を読み込みました: {filename}")
            except Exception as e:
    ...
    def load_autosave(self):
        ...
        if not os.path.exists(self.autosave_folder):
            messagebox.showinfo("情報", "自動保存ファイルがありません")
            return
        ...
        dialog.title("自動保存を読み込み")
        dialog.geometry("600x400")
        dialog.transient(self.root)
        dialog.grab_set()
        
        ttk.Label(dialog, text="自動保存ファイルを選択:", font=('Arial', 12, 'bold')).pack(pady=10)
        ...
        def load_selected():
            ...
            if not selection:
                messagebox.showwarning("警告", "自動保存ファイルを選択してください")
                return
            ...
            try:
                ...
                self.update_songs_display()
                messagebox.showinfo("成功", f"自動保存を読み込みました: {os.path.basename(selected_file)}")
            except Exception as e:
                messagebox.showerror("エラー", f"自動保存読み込み中にエラーが発生しました: {str(e)}")
        
        # Botones
        button_frame = ttk.Frame(dialog)
        button_frame.pack(fill='x', padx=10, pady=10)
        
        ttk.Button(button_frame, text="読み込み", command=load_selected).pack(side=tk.RIGHT, padx=(10, 0))
